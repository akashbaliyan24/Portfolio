        ...
        <motion.nav style={{ opacity: 1 }} className={`fixed top-0 w-full z-50 px-6 py-4 ${isDarkMode ? "bg-gray-950/80" : "bg-gray-50/80"} backdrop-blur-md border-b ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}>
            <div className="max-w-7xl mx-auto flex items-center justify-between">
                ...
                >
                    ...
                    <span className={`text-lg ml-1 ${isDarkMode? "text-gray-400":"text-gray-900"} cursor-default`}>Portfolio</span>
                </motion.div>
                {/* Desktop Navigation  */}
                <div className="hidden md:flex items-center space-x-8 cursor-pointer">
                    ...
                    <motion.button
                        ...
                        className={`p-2 rounded-full transition-colors cursor-pointer ${isDarkMode
                                ? "text-gray-400 hover:text-white hover:bg-gray-800" : "text-gray-600 hover:text-gray-900 hover:bg-gray-200"
                            }`}
                    >
            ...
            <AnimatePresence>
                {isMenuOpen && (
                    ...
                    className={`md:hidden mt-4 p-4 rounded-lg ${
                        ...
                        {["Home","Skills","Work","About","Contact"].map((item)=>(
                            <motion.button key={item} whileHover={{x:5}} onClick={()=>scrollToSection(item.toLowerCase())}
