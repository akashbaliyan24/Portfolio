...
const SkillsSection = () => {
    ...
    return <section ref={sectionRef} id='skills' className={`relative py-24 px-6 ${isDarkMode ? 'bg-gray-950 text-white' : "bg-gray-50 text-gray-950"
        }`}>
        {/* Background Element  */}
        <motion.div style={{ y }} className='absolute inset-0 overflow-hidden'>
            ...
            />
            <div className={`absolute bottom-40 left-1/4 w-64 rounded-full blur-3xl opacity-5 ${isDarkMode ? "bg-purple-500" : "bg-purple-400"
                }`} />
        </motion.div>
        <div className="max-w-6xl mx-auto relative z-10">
            ...
            <motion.div
                ...
                <motion.div className={`text-sm uppercase tracking-widest ${isDarkMode ? "text-gray-500 " : "text-gray-600"} mb-4`} variants={itemVariants}>
                    Technical Expertise
                </motion.div>
                ...
                <motion.p variants={itemVariants} className={`text-lg ${isDarkMode ? "text-gray-400" : "text-gray-600"
                ...
                {SKILLS_CATEGORY.map((category, categoryIndex) => (
                    <motion.div key={category.title} variants={itemVariants} className={`p-8 rounded-2xl border ${isDarkMode ? "bg-gray-900/50 border-gray-800 backdrop-blur-sm" : "bg-white/80 border-gray-200 backdrop-blur-sm"
                        ...
                        <div className="flex items-center mb-6">
                            ...
                            <div className="text-xl font-medium mb-1">{category.title}
            ...
            {/* Additional Skills  */}
            <motion.div initial="hidden" variants={containerVariants} animate = {isInView ? "visible":"hidden"} className='mt-16'>
                <motion.div variants={itemVariants} className='text-center mb-8'>
                    <h3 className="text-xl font-medium mb-4">Also working with</h3>
                </motion.div>
                <motion.div variants={itemVariants} className='flex flex-wrap justify-center gap-3 cursor-default'>
                    {TECH_STACK.map((tech,index)=>(
                        <motion.span key={tech} whileHover={{y:-2, scale:1.05}} className={`px-2 py-2 text-sm rounded-full border transition-all duration-300 ${
                            isDarkMode?"bg-gray-900 border-gray-700 text-gray-300 hover:border-gray-600":"bg-white border-gray-300 text-gray-700 hover:border-gray-400"
                        }`}>
                            {tech}
                        </motion.span>
                    ))}
