...
const ContactSection = () => {
  ...
  const isInView = useInView(sectionRef, { once: true, marging: "-100px" });
  ...
  return (
    <section id='contact' className={`py-24 px-6 ${isDarkMode ? "bg-gray-950 text-white " : "bg-white text-gray-900"
      ...
      <div className="max-w-6xl mx-auto relative z-10 ">
        ...
        <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className='text-center mb-20'>
          ...
          <motion.p variants={itemVariants} className={`text-xl max-w-2xl mx-auto ${isDarkMode ? "text-gray-400" : "text-gray-600"
            ...
            Every great developer starts somewhere — and I'm excited to grow, learn, and create impactful web experiences.
          </motion.p>
        </motion.div>
            ...
            <motion.div variants={itemVariants} className={`p-8 rounded-2xl border ${isDarkMode ? "bg-gray-800/50 border-gray-700 backdrop-blur-sm" : "bg-gray-50/80 border-gray-200 backdrop-blur-sm"
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <TextInPut isDarkMode={isDarkMode} label="Email Address" value={formData.email} handleInputChange={(text) => handleInputChange("email", text)} />
                </div>
                <TextInPut isDarkMode={isDarkMode} label={"Your Message"} value={formData.message} textarea handleInputChange={(text) => handleInputChange("message", text)} />

                <motion.button disabled={isSubmitting} whileHover={{ y: -2, scale: 1.02 }} whileTap={{ scale: 0.98 }} className='w-full bg-blue-500 hover:bg-blue-600 disabled:bg-blue-400 text-white py-4 rounded-xl text-sm uppercase tracking-widest font-medium transition-all duration-300 flex items-center justify-center space-x-2' onClick={handleSubmit}>
                  {isSubmitting ? (
                    <>
          ...
          <motion.div initial="hidden" animate={isInView ? "visible" : "hidden"} variants={containerVariants} className='space-y-8'>
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="space-y-4">
                {CONTACT_INFO.map((info, index) => (
                  <motion.div key={info.label} variants={itemVariants} whileHover={{ x: 4 }} className={`flex items-center space-x-4 p-4 rounded-xl ${isDarkMode ? "bg-gray-800/30 hover:bg-gray-800/50" : "bg-gray-50/50 hover:bg-gray-100/50"
            ...
            <motion.div variants={itemVariants} className={`p-6 rounded-xl border ${isDarkMode ? "bg-green-500/10 border-green-500/20" : "bg-green-50 border-green-200"
              ...
              <p className={`text-sm ${isDarkMode ? "text-gray-400" : 'text-gray-600'
                }`}>I'm currently available for freelance and full-time opportunities.</p>
            </motion.div>
          </motion.div>
        </div>
        {/* Bottom CTA  */}
